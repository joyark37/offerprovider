...
async function translateJob(job) {
  ...
  if (job.jobDescription && /[a-zA-Z]/.test(job.jobDescription)) {
    try {
      const translated = await translator.translateText(job.jobDescription, { targetLang: 'ZH' });
  ...
  if (job.requirements && /[a-zA-Z]/.test(job.requirements)) {
    try {
      const translated = await translator.translateText(job.requirements, { targetLang: 'ZH' });
